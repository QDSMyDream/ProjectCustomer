...
/**
 * Created by MyDraam on 2019/12/2.
 * 显示悬浮栏
 */
public class SectionDecorationRecycleTitle extends RecyclerView.ItemDecoration {

    private static final String TAG = "SectionDecoration";

    private List<CustomerProjectsItem> dataList;

    private DecorationCallbacks callback;
    ...
    public SectionDecorationRecycleTitle(List<CustomerProjectsItem> dataList, Context context, DecorationCallbacks decorationCallback) {
        Resources res = context.getResources();

        Log.e(TAG, "SectionDecorationRecycleTitle: " + dataList.size());
        this.dataList = dataList;
        this.callback = decorationCallback;
        //设置悬浮栏的画笔---paint
        paint = new Paint();
        paint.setColor(res.getColor(R.color.jumbo));
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (pos == 0 || isFirstInGroup(pos)) {
            outRect.top = topGap;
            if (dataList.get(pos).getCustomer().getCome_time() == "") {
                outRect.top = 0;
            }
        } else {
            outRect.top = 0;
    ...
    //定义一个借口方便外界的调用
    public interface DecorationCallbacks {
