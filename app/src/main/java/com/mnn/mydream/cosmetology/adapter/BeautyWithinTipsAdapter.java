...
public class BeautyWithinTipsAdapter extends BaseAdapter implements Filterable {

    private String TAG = "TipsAdapter";
    private List<BeautyBeanKh> mList;
    private ArrayList<BeautyBeanKh> mArray;// 标题列表
    private Context mContext;

    private String setStringColor = "";//要变色的字符串

    public BeautyWithinTipsAdapter(Context context, List<BeautyBeanKh> list) {
        mContext = context;
        mList = list;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.beauty_tip_item_tiplist, null);
        ...
        holder.indexTv1.setGravity(Gravity.CENTER);
        return convertView;
    }
    ...
    public Filter getFilter() {
        ...
        Filter filter = new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (mArray == null) {
                    mArray = new ArrayList<BeautyBeanKh>(mList);
                }

                if (constraint == null || constraint.length() == 0) {
                    ArrayList<BeautyBeanKh> list = mArray;
                    results.values = list;
                    results.count = list.size();
                } else {
                    String prefixString = constraint.toString().toLowerCase();

                    ArrayList<BeautyBeanKh> unfilteredValues = mArray;
                    int count = unfilteredValues.size();

                    ArrayList<BeautyBeanKh> newValues = new ArrayList<BeautyBeanKh>(count);

                    for (int i = 0; i < count; i++) {
                        BeautyBeanKh costorBean = unfilteredValues.get(i);
                        if (costorBean != null) {
...
//                            }
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                mList = (ArrayList<BeautyBeanKh>) results.values;
                if (results.count > 0) {
